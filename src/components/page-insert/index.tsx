...
export default function PageInsert() {
  ...
  const submitAction = useCallback(function () {
    form
      .validateFields()
      .then((res) => {
        fetch('http://localhost:8181/dir/update', {
          method: 'post',
            ...
            {selectData ? (
              <Form form={form}>
                ...
                >
                  ...
                  >
                    <Checkbox value="list">列表页</Checkbox>
                    <Checkbox value="detail">详情页</Checkbox>
                    <Checkbox value="operate">编辑页</Checkbox>
                  </CheckboxGroup>
                </FormItem>
                ...
                {(form.getFieldValue('pageTypes') || []).map((item, index) => (
                  <div style={activeKey === item ? {} : { display: 'none' }}>
